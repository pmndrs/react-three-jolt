...
import type { BodyType, GenerateBodyOptions } from './body-types';
...
// TYPES ========================================
// `BodyType` and `GenerateBodyOptions` live in `body-types.ts` now (issue #165), so `body-state.ts`
// and `RigidBody.tsx` can import them without pulling in `BodySystem` itself. Re-exported here so
// every existing `from './body-system'` / `from '../systems/body-system'` import keeps working.
export type { BodyType, GenerateBodyOptions };
/**
...
// `getThreeObjectForBody` lives in `body-types.ts` now (issue #165) - it never touched
// `BodySystem`, so it moved out with `BodyType`/`GenerateBodyOptions` to break this file's
// import cycle with `body-state.ts`. Re-exported so existing `from './body-system'` imports
// keep working.
export { getThreeObjectForBody } from './body-types';
