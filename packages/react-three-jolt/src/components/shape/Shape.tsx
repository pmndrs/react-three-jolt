...
import { shapeContext as shapeReactContext } from './context';
...
export interface ShapeProps extends Omit<ShapeOptions, 'children'> {
    ...
    /** The description this `<Shape>` built and the Jolt shape it owns. */
    ref?: React.Ref<ShapeHandle>;
}
...
// `ShapeContextValue` was an alias worked around `ShapeContext` naming both a type and a value
// (#148); now that the value is `shapeContext`, the collision - and the need for the alias - is
// gone. It is kept, `@deprecated`, for one release; `ShapeContext` itself is not deprecated.
/** @deprecated Use {@link ShapeContext} - the value/type collision it worked around is fixed. */
export type { ShapeContext, ShapeContext as ShapeContextValue } from './context';
...
// React 19 native convention (#49): `ref` lives on `ShapeProps` as an ordinary prop, so this is
// a plain function component - no `forwardRef` wrapper.
export const Shape = memo(function Shape(props: ShapeProps) {
    const {
        ref: forwardedRef,
        children,
        dynamic = false,
    ...
    useJolt();
    const ref = useForwardedRef<ShapeHandle>(forwardedRef ?? null, {
    ...
    const parentShape = useContext(shapeReactContext);
    ...
    const editLiveCompound = useCallback(
        (edit: (compound: Jolt.Shape) => void): boolean => {
            ...
            if (!dynamicRef.current || !compound || !isMutableCompoundShape(compound)) return false;
    ...
    return <shapeReactContext.Provider value={context}>{children}</shapeReactContext.Provider>;
});
