...
import { useBodyEvent, useForwardedRef, useJolt } from '../hooks';
import {
    type AutoShape,
    type BodyState,
...
import { shapeContext as shapeReactContext } from './shape/context';

/**
 * `...objectProps` (everything left after destructuring the props below) is spread onto the
 * `<object3D>` this component renders, so the type has to admit ordinary object3D props
 * (`visible`, `castShadow`, `userData`, event handlers, ...) - issue #148. The props RigidBody
 * gives its own, incompatible meaning to (`position`/`rotation`/`scale`/`quaternion`, applied to
 * the *body* rather than passed through as JSX, plus `ref`/`children`) are omitted from the
...
// Named `rigidBodyContext` (lowercase), distinct from the `RigidBodyContext` *type* above
// (#148): the two used to share one identifier, a type and a value colliding under the same
// name.
...
// React 19 native convention (#49): `ref` is an ordinary prop on `RigidBodyProps`, so this is a
// plain function component - no `forwardRef` wrapper. `memo` is kept: every `<RigidBody>` sits
// under a `<Physics>` tree that can re-render often (camera rigs, controllers), and this
// component's own state (`activeShape`/`body`) already governs when it needs to re-render.
// initial version from r3/rapier
export const RigidBody = memo(function RigidBody(props: RigidBodyProps) {
    const {
        ref: forwardedRef,
        children,
    ...
    const autoShape: AutoShape | undefined =
        colliders === false ? undefined : colliders ? COLLIDERS_TO_AUTO_SHAPE[colliders] : shape;
    ...
    useEffect(() => {
        if (body && composedDescriptor.current) body.shapeDescriptor = composedDescriptor.current;
    }, [body]);
    ...
    // Teardown, on unmount only (#57: was `useUnmount`, now a plain `useEffect` whose only
    // job is its cleanup - deps `[]` so it runs exactly once per mount, matching the old
    // hook's contract exactly).
    useEffect(() => {
        return () => {
            // A RigidBody whose shape children never resolved has no body at all; this used
            // to throw on unmount trying to read `.handle` of undefined.
            const current = rigidBodyRef.current as BodyState | undefined;
            if (current) bodySystem.removeBody(current.handle);
            // #155: the compound this component composed out of its colliders is ours to
            // release
            releaseShape(composedShape.current);
    ...
    // Shape update
    useEffect(() => {
        // #57 audit: this used to check `!bodyLoaded` - a ref *object*, always truthy - which
        // never actually gated anything (the real guard was always `!rigidBodyRef.current`).
        // Fixed to read `.current` for clarity; behavior is unchanged either way, since the
        // two only ever become true/false together (see the body-creation effect above).
        if (!rigidBodyRef.current || !bodyLoaded.current) return;
        const body = rigidBodyRef.current as BodyState;
    ...
    useEffect(() => {
        ...
        if (allowObstruction !== undefined) {
            ...
            // #57 audit: `!== undefined`, not truthy - `obstructionTimelimit={0}` (expire
            // immediately) is a legitimate value and used to be silently dropped.
            if (obstructionTimelimit !== undefined) {
    ...
    useEffect(() => {
        ...
        // #57 audit: `activeShape` was missing here (unlike the identical-shaped Groups
        // effect above, which explains why it needs it). Without it, a body whose shape
        // comes from `<Shape>` children - created a render *after* this effect's first,
        // early-returning run - never got `dof`/`lockRotations`/`lockTranslations` applied at
        // all, because none of this effect's other deps change once the body exists.
    }, [dof, lockRotations, lockTranslations, activeShape, rigidBodyRef]);
    ...
    }, [body, type, position, rotation, scale, quaternion, notifyShapeChanged, declareCollider]);
    ...
    const shapeContext = useMemo(
        () =>
            combining
                ...
                : undefined,
        [combining, activeShape, addShape, modifyShape, removeShape, isSoleShape, declareCollider]
    );

    return (
        <rigidBodyContext.Provider value={contextValue}>
            <shapeReactContext.Provider value={shapeContext}>
                <object3D ref={objectRef} {...objectProps}>
                    {children}
                </object3D>
            </shapeReactContext.Provider>
