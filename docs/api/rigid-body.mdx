...
| `isStatic` | read-only |
| `saveState()` | returns a [`PhysicsSnapshot`](/api/physics#physicssnapshot) of just this body |
| `restoreState(snapshot)` | restores it and resyncs the three.js object + interpolation cache immediately, sleeping included |
