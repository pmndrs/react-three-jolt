...
| `motionQuality` | `'discrete' \| 'linearCast'` | collision detection quality. `'discrete'` (Jolt's default) only tests at the start/end of a step; `'linearCast'` sweeps the body for the step, so a small fast body doesn't tunnel through thin geometry. Costs more. Reactive |
...
`motionQuality` (`'discrete' | 'linearCast'`) is the same idea, but goes through
`BodyInterface.SetMotionQuality` rather than a `MotionProperties` setter — jolt-physics exposes
`MotionProperties.GetMotionQuality()` but no matching setter on it, only on `BodyInterface`. A
no-op on a static body, which has no `MotionProperties` at all.
...
`<InstancedRigidBodies>` replaces `<instancedMesh>`: the child mesh describes the shape and
