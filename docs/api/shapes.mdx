...
These are the types that can be inferred from — or forced onto — a geometry. `compound` is an
alias of `staticCompound`. The [descriptor pipeline](#the-shape-pipeline) has a slightly larger
set, `ShapeType`, which adds `staticCompound`, `mutableCompound`, `scaled` and
`offsetCenterOfMass`.
...
`dynamicMeshStrategy` is a **body option**, not a `<RigidBody>` or `<Physics>` prop — today the
only way to set it is to create the body yourself:
...
| prop | type | |
| --- | --- | --- |
| `type` | `AutoShape \| ShapeType` | default `'box'` |
...
> [!NOTE]
> The tags are `convex` and `trimesh` — there is no `convexHull` or `mesh`. `compound` exists only
> in [`AutoShape`](#autoshape), where it is normalised to `staticCompound`.
