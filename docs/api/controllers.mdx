...
| prop | type | default | |
| --- | --- | --- | --- |
| `type` | `'fourWheel' \| 'twoWheel' \| 'tracked'` | `'fourWheel'` | a car, a motorcycle or a tank |
| `name` | `string` | `'car'` / `'bike'` / `'tank'` | the key it is registered under in its `VehicleSystem` |
...
order is `fl, fr, bl, br` for a four-wheeler, `front, back` for a two-wheeler and, for a tracked
vehicle, the left track front-to-back followed by the right track front-to-back (`l0, l1, l2, l3,
r0, r1, r2, r3` at the default `wheels.count`) — see `wheelOrderByType`. A tracked wheel never
steers, so its rotation only ever spins about its own axle. If both `wheels` and `wheelObjects`
are given, `wheels` wins.
...
`FourWheelVehicleManager`, `TwoWheelVehicleManager` and `TrackedVehicleManager` are the three
concrete classes; `VehicleSystem` holds them by name (`addVehicle`, `getVehicle`, `removeVehicle`,
`setPosition`, `detachFromLoop`, `destroy`).

### Settings

`vehicleSettings` is a discriminated union on `type`. Shared by all three:
...
`bodyObject`, `wheelObjects`. (`twoWheel` declares the same two engine fields but its manager does
not read them yet — the motorcycle's engine is still the fixed tuning jolt's own example uses.)
...
**`tracked`** (jolt's `TrackedVehicleController` — issue #246) adds `left` / `right` track
settings and a `wheels` object. A track
(`{ drivenWheel, inertia, angularDamping, maxBrakeTorque, differentialRatio }`) is jolt's
`VehicleTrackSettings`: `drivenWheel` indexes into *that track's own* wheel list (0, the
frontmost wheel, by default), not the constraint's global wheel list. `wheels.count` (default 4)
lays that many wheels evenly along each side, front to back; there is no per-corner override, only
the shared radius/width/suspension/friction settings and `longitudinalFriction` /
`lateralFriction` (jolt's `WheelSettingsTV` — a tracked wheel never steers, so it has none of the
`fourWheel` wheel's steering/brake torque properties). Defaults: 5.0 × 2.6 × 0.7 m, 4000 kg,
cylinder cast, 2000 N·m of engine torque, 0.4 m wheels.

Driving a tracked vehicle steers by skidding: `move()`'s steering axis is mixed into jolt's
`leftRatio`/`rightRatio` (`SetDriverInput(forward, leftRatio, rightRatio, brake)`) rather than
forwarded as a wheel angle — the outside track speeds up and the inside one slows (or reverses,
for a turn sharp enough to pivot in place) by the same amount, so driving straight always asks
both tracks for exactly `forward`.
...
`maxHandBrakeTorque` (four-wheel), `posZ` / `suspensionFreq` / `brakeTorque` (two-wheel) or
`longitudinalFriction` / `lateralFriction` (tracked).

`defaultFourWheelVehicleSettings`, `defaultTwoWheelVehicleSettings`,
`defaultTrackedVehicleSettings` and `resolveVehicleSettings(settings)` are exported if you want to
inspect or pre-merge them. Your settings are merged **over** the defaults — a motorcycle built
with a custom mass or custom wheels no longer silently gets the defaults back.
...
```tsx
import { TrackedVehicle } from '@react-three/jolt/controllers';
...
`<TrackedVehicle>` is exactly `<Vehicle type="tracked">` under its own name — every prop, every
secondary physics option, `useVehicle({ type: 'tracked' })`, all of it. `TrackedVehicleManager` is
the concrete class (`declare controller: Jolt.TrackedVehicleController`).
