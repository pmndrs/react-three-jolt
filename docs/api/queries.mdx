...
Five query objects, all created from the physics system and all following the same shape: set up
...
| [`PointCollider`](#collidepoint) / [`useCollidePoint`](#collidepoint) | which bodies contain this single point *right now* (Jolt's `CollidePoint`) |
...
`CollidePoint` asks the simplest overlap question of all: not "what does this shape overlap",
just "which bodies contain this single point, right now" — a spawn-point check, a click-to-select
test against 3D volumes rather than the three.js scene graph, a trigger volume shaped like
whatever you already have instead of a sensor body.

```tsx
import { useCollidePoint } from '@react-three/jolt';
import { useEffect } from 'react';

export function SpawnCheck() {
    const collider = useCollidePoint([0, 1, 0]);

    useEffect(() => {
        const result = collider.cast();
        if (result) console.log('occupied');
    }, [collider]);
...
`useCollidePoint(point?, type?)` creates a `PointCollider`, applies the arguments, and destroys it
when the component unmounts — or whenever `point`/`type` change, mirroring `useRaycaster` (compare
by identity, so hoist an inline array/`Vector3`). There is no hook-less form needed for the simple
case, but you can still take one from the physics system yourself:

```ts
import { useJolt } from '@react-three/jolt';

const { physicsSystem } = useJolt();
const collider = physicsSystem.getPointCollider();
```

`point` accepts the same `anyVec3` shapes as `Raycaster.origin`/`direction` (a `THREE.Vector3`, a
`number[]`, or a Jolt vector) and is rebuilt in place on every write — no allocation per frame, so
it's safe to drive from `useFrame`.

`setCollector('closest' | 'any' | 'all')` works exactly as it does for the other queries.
`cast()` returns the single hit when there is exactly one, the `hits` array when there are more,
and `false` on a miss — same contract as `ShapeCollider.cast()`.

### `PointCollisionResult`

Jolt's own `CollidePointResult` carries only a body and a sub-shape id — no contact point or
penetration depth, since a point is either inside a shape or it isn't:

| member | |
| --- | --- |
| `bodyHandle` | pass to `bodySystem.getBody()` |
| `subShapeId2` | **a raw `Jolt.SubShapeID` owned by the collector** — read `.GetValue()`, never destroy |
...
They now all extend a small hierarchy instead — useful mainly if you're building another query
type (`PointCollider` — issue #248 — is one such addition, following the same pattern as
`ShapeCollider`), or want to know exactly what `destroy()` covers:
...
`Raycaster`); `ShapeCollider`, `PointCollider` and `Multicaster` extend `QueryBase` directly since
none of them uses the ray-cast collector family. `RaycastHit` and `ShapecastHit` both extend
`HitBase`, which is why their fields line up exactly (see [`RaycastHit`](#raycasthit) above) -
`PointCollisionResult` does not, since `CollidePoint`'s result has no start/end/position at all.
...
| `useCollidePoint()` | unmount only, and whenever `point` / `type` change — a change to either builds a new `PointCollider` and destroys the old one, same as `useRaycaster` |
...
`useRaycaster`'s `origin` and `direction` (and `useCollidePoint`'s `point`) are compared by
identity, so passing an inline array or `Vector3` re-creates the caster/collider on every render.
Hoist them, or use `raycaster.set(...)` / `pointCollider.point = ...`.

- Anything from `physicsSystem.getShapecaster()` / `getShapeCollider()` / `getPointCollider()` /
  `getRaycaster()` is yours: pair it with `useEffect(() => () => caster.destroy(), [caster])`.
- `destroy()` is idempotent on every query type — see [Shared base](#shared-base). `ShapeCollider`
  and `PointCollider` go one step further and also null out every field they freed, so a stray
  call afterwards fails loudly instead of touching freed memory; the others just no-op on a
  second call.
- There is no `useShapecaster` / `useShapeCollider` hook yet — `useCollidePoint` (issue #248) is
  the first hook wrapper for one of the `QueryBase`-direct query types.
