...
The debug objects are **pooled per raycaster**: one shared material per drawing type, geometry
...
> [!WARNING]
> `Shapecaster` still has the **old, unpooled** debug drawing: it allocates a geometry, a material
> and an `Object3D` on every call, its markers are world-axis-aligned crosses, and
> `clearDebugging()` disposes nothing. Turn shapecast debugging on for a look, not for a session.
...
- `destroy()` is idempotent on `ShapeCollider` — it also nulls out every field it freed, so a
  stray call afterwards fails loudly instead of touching freed memory. Don't rely on that for the
  others.
