...
- **[`<Water>` / `useBuoyancy()`](/api/buoyancy).** Box shaped water volumes built on
  `Body.ApplyBuoyancyImpulse` — every dynamic body whose bounds overlap one floats, drags and
  rides a `flow` current, and a sleeping body is woken so it can float back up. `buoyancy` is a
  ratio to gravity (`1` neutral, `>1` floats, `<1` sinks), not a per-body density — see the doc for
  how to simulate different densities with `group`/`filter` instead.
